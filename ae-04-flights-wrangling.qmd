...
Each row represents a flight departing from a NYC airport in 2013.
...
If the dep_delay is negative then the flight departed earlier then expected.
...
Add response here.
This code snippet keeps all the flights that meet both the conditions of having a 
destination of RDU or GSO and the arrival delay or departure delay is less then 0 (arrived early.
...
flights |> 
  ...
  group_by(origin) |>
  summarize(proportion_to_rdu = mean(rdu_bound))
```
...
flights |>
  group_by(origin) |>
  summarize(median_dep_delay = mean(dep_delay, na.rm = TRUE)) |>
...
plane_info <- flights |>
  ...
  summarize(
    ...
    standard_deviation_dist = sd(distance, na.rm = TRUE)
  )
...
bind_rows(bottom5, top5)
```

I used bind rows for the last question and found information on how to use bind rows here:
https://dplyr.tidyverse.org/reference/bind_rows.html 
This is from the tidyverse and dplyr website.
